...
.next {
  grid-area: next;
  background: url("https://i7.pngguru.com/preview/361/489/274/bullet-computer-icons-arrow-next-button.jpg")
    center no-repeat;
...
.fav {
  grid-area: fav;
  background: url("https://purepng.com/public/uploads/medium/heart-icon-s4k.png")
    no-repeat center;
  background-size: 70px;
  justify-self: end;
}
...
This style block uses some paths to external images hosted on GitHub, rather than to a relative path. This is because CodeSandbox doesn't host images easily; normally you'd just add an image on a relative path such as `/images/myImage.png`.
